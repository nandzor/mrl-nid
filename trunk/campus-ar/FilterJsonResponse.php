...
$response = array();
    ...
    if ( empty( $response["hotspots"] ) ) {
        $response["errorCode"] = 20;
         $response["errorString"] = "No POI found. Please adjust the range and Make sure you have Checked atleast 1 filter option";
    }
    else {
...
$querystr= Conv($value['CHECKBOXLIST']);

if($querystr!="checkboxlist parameter is not passed in GetPOI request")
{
...
function Conv( $checkboxlist ) {
  	...
  	if( isset( $checkboxlist ) )
 	{
    		...
    		if ( !empty( $checkboxlist ) ) 
		{
			...
			return "checkboxlist parameter is not passed in GetPOI request";
  	}
